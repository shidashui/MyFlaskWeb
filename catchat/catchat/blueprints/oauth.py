...
#oauth1 上边是oauth2版本区别在request_token_url
...
def oauth_login(provider_name):
    ...
    return providers[provider_name].authorize(callback=callback)          #请求授权，并回调
...
def oauth_callback(provider_name):
    ...
    response = provider.authorized_response() #交换access
...
def get_social_profile(provider, access_token):
    ...
    response = provider.get(profile_endpoint, token=access_token)  #获取资料
    print(response.data)
    ...
    if provider.name == 'twitter':
        ...
        email = response.data.get('email', '164635470@qq.com')
